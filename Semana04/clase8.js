...
function mostrarMensajeExito(listado) {
    if (listado.length === 0) {
        const mensajeDeExitoDiv = document.createElement("div");
        mensajeDeExitoDiv.setAttribute("id", "exito");
        mensajeDeExitoDiv.style = "background:rgba(0, 255, 0, 0.2);padding:.5em 1em;color: green;margin:0.5em 0";
        mensajeDeExitoDiv.innerHTML = `<p><span>¡Formulario completado con éxito!</span></p>`;
        form.appendChild(mensajeDeExitoDiv);

        const submitButton = document.querySelector("button[type='submit']");
        submitButton.disabled = true;

        setTimeout(() => {
            mensajeDeExitoDiv.remove();
